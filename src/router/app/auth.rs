...
pub async fn login_form(
    ...
    let result = state.db.query(
        "SELECT users.id, users.email, users.password, users.created_at, settings.openai_api_key FROM users LEFT JOIN settings ON settings.user_id=users.id WHERE users.email = ?",
